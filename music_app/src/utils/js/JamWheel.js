class JamWheel {
  ...
  updateKeys(type) {
    ...
    for (let angle = 270; angle < 630; angle = angle + pointAngle) {
      ...
      switch (type) {
        case FatOscillator.type:
          var tri = new FatOscillator(
            this.p5,
            this.circenterX,
            this.circenterY,
            x + this.circenterX,
            y + this.circenterY,
            this.p5.cos(this.p5.radians(temp)) * radius + this.circenterX,
            this.p5.sin(this.p5.radians(temp)) * radius + this.circenterY,
            this.hex_color_red[counter],
            this.OCTAVE_LOWER[counter],
          )
        ...
        case Kalimba.type:
          var tri = new Kalimba(
            ...
            this.p5.cos(this.p5.radians(temp)) * radius + this.circenterX,
            this.p5.sin(this.p5.radians(temp)) * radius + this.circenterY,
            this.hex_color_green[counter],
            this.C_MAJ_SCALE[counter],
          )
          this.keyWheel.push(tri)
